...
# The corresponding function load function for the save dictionary generated by
# the thing_tracker.save() function in ThingTracker.gd
func load_state(save_dict: Dictionary):
	for key in save_dict.keys():
		var thing_save_dict = save_dict[key]
		if thing_save_dict is Dictionary and thing_save_dict.has_all(["grid_position", "thing_id"]):
			var cellv : Vector2 = str2var(thing_save_dict["grid_position"])
			_load_thing(cellv, thing_save_dict)
		

# A function called by load_state, used to place things and reload their states. 
func _load_thing(cellv: Vector2, save_dict: Dictionary):
	if save_dict.has_all(["thing_id"]):
		var new_thing: Node2D = BuildingList.things[save_dict["thing_id"]].instance()
		if save_dict["thing_id"] == "Pipe":
			_flat_things.add_child(new_thing)
		else:
			add_child(new_thing)
		...
		if new_thing.has_method("load_state"):
			new_thing.load_state(save_dict)
		_thing_tracker.place_thing(new_thing, cellv)
